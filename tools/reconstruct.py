...
def chamfer_distance(x: torch.Tensor, y: torch.Tensor) -> float:
    ...
    计算对称 Chamfer 距离
    参数:
        x: Tensor[B, N, 3] 或 Tensor[N, 3]
        y: Tensor[B, N, 3] 或 Tensor[N, 3]
    返回:
...
def save_results_json(cds, args, save_dir, timestamp_str):
    """将均值/方差与超参数写入 JSON"""
    n = len(cds)
    mean_cd = float(np.mean(cds)) if n > 0 else float('nan')
    # 使用无偏方差（样本方差，ddof=1）；若 n<2 则置 0.0
    var_cd = float(np.var(cds, ddof=1)) if n > 1 else 0.0
    std_cd = float(math.sqrt(var_cd)) if n > 1 else 0.0
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        cds = evaluate(tokenizer, estimator, dataloader, device, save_dir)

    # 统计并写 JSON
    save_results_json(cds, args, save_dir, timestamp)

    # 控制台也输出一下均值
    if len(cds) > 0:
        print(f"✅ Chamfer Distance over {len(cds)} samples: mean={np.mean(cds):.6f}, var={np.var(cds, ddof=1) if len(cds)>1 else 0.0:.6f}")
    else:
        print("⚠️ 未得到有效的 Chamfer Distance 样本。")
